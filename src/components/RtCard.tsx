...
import { CheckCircle2, Circle, FileText, Eye, Download, Trash2, Plus, GraduationCap } from 'lucide-react';
...
export function RtCard({ rt, petaList, assignments, onView, onAddFile, onChanged }: RtCardProps) {
  ...
  return (
    <motion.div
      ...
      onClick={onView}
      className={`group cursor-pointer rounded-xl border bg-white p-4 shadow-sm dark:bg-slate-800 ${cardBorder}`}
    >
      {/* Header */}
        ...
        <div className="mb-2 flex flex-wrap gap-1.5">
          {visibleBadges.map((badge, i) => {
            ...
            return (
              <span
                key={i}
                className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ${color?.badge || 'bg-slate-100 dark:bg-slate-700'} ${color?.badgeText || 'text-slate-600 dark:text-slate-300'}`}
              >
                <GraduationCap size={10} className="shrink-0" />
                <span className="max-w-[120px] truncate">{badge.name}</span>
              </span>
            );
          })}
          {extraCount > 0 && (
            <span className="inline-flex items-center rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-500 dark:bg-slate-700 dark:text-slate-400">
              +{extraCount}
            </span>
          )}
        </div>
      ...
      {isDone ? (
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5 pt-1">
            <button
              ...
              className="flex items-center gap-1 rounded-md bg-teal-600 px-2.5 py-1.5 text-xs font-medium text-white transition hover:bg-teal-700"
            >
              <Eye size={12} /> Lihat
            </button>
            <button
              onClick={(e) => handleDownload(e, petaList[0])}
              className="flex items-center gap-1 rounded-md border border-slate-300 px-2.5 py-1.5 text-xs font-medium text-slate-600 transition hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700"
            >
            ...
            {session && (
              <button
                ...
                className="flex items-center gap-1 rounded-md border border-red-200 px-2.5 py-1.5 text-xs font-medium text-red-600 transition hover:bg-red-50 dark:border-red-800 dark:text-red-400 dark:hover:bg-red-900/20"
              >
        ...
        <div className="py-2">
          {session ? (
            ...
            <div className="flex items-center justify-center py-2.5 text-xs text-slate-400 dark:text-slate-500">
